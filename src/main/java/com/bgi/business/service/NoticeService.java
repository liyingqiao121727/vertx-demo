...
public class NoticeService {
    ...
    private static InternalLogger logger = InternalLoggerFactory.getInstance(NoticeService.class);
        ...
        SQLClientHelper.inTransactionSingle(rxSqlClientW, conn -> {
            return conn.rxUpdateWithParams(sql, params).flatMap(updateResult -> {
                ...
                if (null != userIds && userIds.size() > 0) {
                    List<NoticeUser> users = new LinkedList<>();
                    ...
                    for (int i = 0; i < customUserIds.size(); i++) {
                        ...
                        noticeUser.setUserId(customUserIds.getString(i));
                        noticeUser.setRead(Constant.NOTICE_UNREAD);
                        users.add(noticeUser);
                    }
                    params.clear();
                    String userSql = NoticeUser.insertBatch(users, params);
                    logger.info(userSql);
    ...
    public void insertMany(JsonArray notices, RoutingContext routingContext) {
        ...
        List<NoticeUser> noticeUserList = new LinkedList<>();
        Notice notice = null;
        ...
        for (int i = 0; i < notices.size(); i++) {
            ...
            list.add(notice);
            userIds = notice.getCustomUserIds();
            ...
            for (int j = 0; j < userIds.size(); j++) {
                user = new NoticeUser();
                user.setUserId(userIds.getString(j));
                user.setNoticeId(notice.getId());
                noticeUserList.add(user);
            }
        }
        JsonArray params = new JsonArray(new LinkedList());
        ...
        SQLClientHelper.inTransactionSingle(rxSqlClientW, conn -> {
            return conn.rxUpdateWithParams(sql, params).flatMap(updateResult -> {
                ...
                if (noticeUserList.size() > 0) {
                    String sqlUser = NoticeUser.insertBatch(noticeUserList, paramsUser);
                    logger.info(sqlUser);
